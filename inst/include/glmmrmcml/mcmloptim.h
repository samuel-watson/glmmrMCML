...
public:
  ...
  D_(D),Z_(Z), X_(X), y_(y), 
  u_(u), family_(family) , link_(link),
  start_(start), trace_(trace) {
    P_ = X_.cols();
    Q_ = D->data_->n_cov_pars();
    cov_par_fix_ = start.segment(P_,Q_);// start.subvec(P_,P_+Q_-1);
    beta_ = start.segment(0,P_); //start.subvec(0,P_-1);
    theta_ = start.segment(P_,Q_);// start.subvec(P_,P_+Q_-1);
    // Rcpp::Rcout << "\n beta:" << beta_;
    // Rcpp::Rcout << "\n theta:" << beta_;
  ...
  void d_optim(){
    glmmr::likelihood::D_likelihood<T> ddl(D_,u_);
  ...
  void l_optim(){
    glmmr::likelihood::L_likelihood ldl(Z_,X_,y_,u_,family_,link_);
    Rbobyqa<glmmr::likelihood::L_likelihood> opt;
    opt.control.iprint = trace_;
    std::vector<double> start_b;
    start_b.resize(P_);
    Eigen::VectorXd::Map(&start_b[0], beta_.size()) = beta_;
    //std::vector<double> start_b(start_.data(),start_.data()+P_);//arma::vec start_b = start_.subvec(0,P_-1);
    ...
    beta_ = Eigen::Map<Eigen::ArrayXd>(beta.data(),P_);//opt.par().subvec(0,P_-1);
    if(family_=="gaussian") sigma_ = beta[P_];
  }
  
  // jointly optimise marginal y and random effects models
  void f_optim(){
    glmmr::likelihood::F_likelihood<T> dl(D_,Z_,X_,y_,u_,cov_par_fix_,family_,link_,true,true,sigma_);
    ...
    std::vector<double> beta = opt.par();
    beta_ = Eigen::Map<Eigen::ArrayXd>(beta.data(),P_);//beta_ = pars(0,P_-1);
    theta_ = Eigen::Map<Eigen::ArrayXd>(beta.data()+P_,Q_);//theta_ = pars(P_,P_+Q_-1);
    if(family_=="gaussian") sigma_ = beta[P_+Q_];
  }
  
  // newton raphson step
  void mcnr(){
    //generate residuals
    Eigen::VectorXd xb = X_*beta_;
    Eigen::ArrayXd sigmas(niter_);
    // will have to stack XtXW rather than use slices
    Eigen::MatrixXd XtXW = Eigen::MatrixXd::Zero(P_*niter_,P_);
    Eigen::MatrixXd Wu = Eigen::MatrixXd::Zero(n_,niter_);
    //check if parallelisation is faster or not! if not then declare vectors in advance
#pragma omp parallel for
    for(int i = 0; i < niter_; ++i){
      Eigen::VectorXd zd = Z_ * u_.col(i);
      zd = glmmr::maths::mod_inv_func(xb + zd, link_);
      Eigen::ArrayXd resid = (y_ - zd).array();
      sigmas(i) = std::sqrt((resid - resid.mean()).square().sum()/(resid.size()-1));
      Eigen::VectorXd wdiag = glmmr::maths::dhdmu(xb + zd,family_,link_);
      for(int j=0; j<n_; j++){
        double var = (family_=="gaussian" || family_=="gamma") ? sigmas(i) : 1.0;
        XtXW.block(P_*i, 0, P_, P_) +=  (var/(wdiag(j)*wdiag(j)))*(X_.row(j).transpose()*X_.row(j));
        Wu(j,i) += (var/wdiag(j))*resid(j);
      }
    }
    XtXW *= (double)1/niter_;
    Eigen::MatrixXd XtWXm = XtXW.block(0,0,P_,P_);
    for(int i = 1; i<niter_; i++) XtWXm += XtXW.block(i*P_,0,P_,P_);
    XtWXm = XtWXm.inverse();
    Eigen::VectorXd Wum = Wu.rowwise().mean();
    Eigen::VectorXd bincr = XtWXm*X_.transpose()*Wum;
    beta_ += bincr;
    sigma_ = sigmas.mean();
  }
  ...
  Eigen::MatrixXd b_hessian(double tol = 1e-4){
    glmmr::likelihood::L_likelihood hdl(Z_,X_,y_,u_,family_,link_,true,sigma_);
    //herre
    std::vector<double> ndep;
    for(int i = 0; i < P_; i++) ndep.push_back(tol);
    hdl.os.ndeps_ = ndep;
    std::vector<double> hessian(P_*P_,0.0);
    std::vector<double> start_b;
    for(int i = 0; i < P_; i++) start_b.push_back(beta_(i));
    hdl.Hessian(start_b,hessian);
    Eigen::MatrixXd hess = Eigen::Map<Eigen::MatrixXd>(hessian.data(),P_,P_);
    return hess;
  }
  

  Eigen::VectorXd f_grad(double tol = 1e-4){
    glmmr::likelihood::F_likelihood<T> fdl(D_,Z_,X_,y_,u_,cov_par_fix_,family_,link_,false,true,sigma_);
    fdl.os.usebounds_ = 1;
    std::vector<double> lower = lower_b_;
    ...
    fdl.os.upper_ = upper;
    std::vector<double> ndep;
    for(int i = 0; i < (P_+Q_); i++) ndep.push_back(tol);
    fdl.os.ndeps_ = ndep;
    std::vector<double> gradient(P_+Q_,0.0);
    fdl.Gradient(start_b,gradient);
    Eigen::VectorXd grad = Eigen::Map<Eigen::VectorXd>(gradient.data(),P_+Q_);
    return grad;
  }
  
  Eigen::MatrixXd f_hess(double tol = 1e-4){
    glmmr::likelihood::F_likelihood<T> fhdl(D_,Z_,X_,y_,u_,cov_par_fix_,family_,link_,false,true,sigma_);
    ...
    for(int i = 0; i < (P_+Q_); i++) ndep.push_back(tol);
    fhdl.os.ndeps_ = ndep;
    std::vector<double> hessian((P_+Q_)*(P_+Q_),0.0);

    fhdl.Hessian(start_b,hessian);
    Eigen::MatrixXd hess = Eigen::Map<Eigen::MatrixXd>(hessian.data(),P_+Q_,P_+Q_);
    return hess;
  }
